...
<p>
    ...
    <input type="text" id="<?php echo $this->get_field_id( 'posts_per_page' ); ?>" name="<?php echo $this->get_field_name( 'posts_per_page' ); ?>" value="<?php echo esc_attr($instance['args']['posts_per_page']); ?>" class="widefat" style="width:50px!important" /> <?php _e('posts', 'recently'); ?>
</p>

<p style="margin-bottom:0;">
    <label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e('Offset', 'recently'); ?>:</label><br />
    <input type="text" id="<?php echo $this->get_field_id( 'offset' ); ?>" name="<?php echo $this->get_field_name( 'offset' ); ?>" value="<?php echo esc_attr($instance['args']['offset']); ?>" class="widefat" style="width:50px!important" /><br />
...
<p>
    ...
    <input type="text" id="<?php echo $this->get_field_id( 'post_type' ); ?>" name="<?php echo $this->get_field_name( 'post_type' ); ?>" value="<?php echo esc_attr(implode(',', $instance['args']['post_type'])); ?>" class="widefat" /><br />
...
<p>
    ...
    <input type="text" id="<?php echo $this->get_field_id( 'post_id' ); ?>" name="<?php echo $this->get_field_name( 'post_id' ); ?>" value="<?php echo ( isset($instance['args']['post__not_in']) ) ? esc_attr(implode(',', $instance['args']['post__not_in'])) : ''; ?>" class="widefat" /><br />
...
<p class="taxonomy-filter">
    ...
    if ( $taxonomies ) {
        foreach ( $taxonomies  as $taxonomy ) {
            ...
            echo '<label><input type="radio" name="' . $this->get_field_name( 'taxonomy' ) . '" value="' . esc_attr($taxonomy->name) . '"' . ( (isset($instance['args']['tax_query']) && isset($instance['args']['tax_query'][0]) && $taxonomy->name == $instance['args']['tax_query'][0]['taxonomy']) ? ' checked' : '') . '> ' . esc_html($taxonomy->labels->singular_name) . '</label><br>';
        }
    }
    ...
    <div class="tax-field" style="display: <?php echo isset($instance['args']['tax_query']) ? 'block' : 'none'; ?>; width: 90%; margin: 10px 0; padding: 3% 5%; background: #f5f5f5;">
        <span class="tax-id-field"<?php echo ( isset($instance['args']['tax_query']) && 'post_format' == $instance['args']['tax_query'][0]['taxonomy'] ) ? ' style="display:none;"' : ''; ?>>
            ...
            <input type="text" id="<?php echo $this->get_field_id( 'tax_id' ); ?>" name="<?php echo $this->get_field_name( 'tax_id' ); ?>" value="<?php echo esc_attr(implode(',', $tax_terms)); ?>" class="widefat" /><br />
        ...
        <span class="tax-slug-field"<?php echo ( isset($instance['args']['tax_query']) && 'post_format' != $instance['args']['tax_query'][0]['taxonomy'] ) ? ' style="display:none;"' : ''; ?>>
            ...
            <input type="text" id="<?php echo $this->get_field_id( 'tax_slug' ); ?>" name="<?php echo $this->get_field_name( 'tax_slug' ); ?>" value="<?php echo esc_attr(implode(',', $tax_terms)); ?>" class="widefat" /><br />
...
<p style="margin-bottom:0;">
    ...
    <input type="text" id="<?php echo $this->get_field_id( 'author_id' ); ?>" name="<?php echo $this->get_field_name( 'author_id' ); ?>" value="<?php echo esc_attr(implode(',', $author_ids)); ?>" class="widefat" /><br />
...
<div class="shorten-title" style="display:<?php if ($instance['shorten_title']['active']) : ?>block<?php else: ?>none<?php endif; ?>; width:90%; margin:10px 0; padding:3% 5%; background:#f5f5f5;">
    <label for="<?php echo $this->get_field_id( 'shorten_title-length' ); ?>"><?php _e('Shorten title to', 'recently'); ?> <input type="text" id="<?php echo $this->get_field_id( 'shorten_title-length' ); ?>" name="<?php echo $this->get_field_name( 'shorten_title-length' ); ?>" value="<?php echo esc_attr($instance['shorten_title']['length']); ?>" class="widefat" style="width:50px!important" /></label><br />
...
<div class="excerpt" style="display:<?php if ($instance['post-excerpt']['active']) : ?>block<?php else: ?>none<?php endif; ?>; width:90%; margin:10px 0; padding:3% 5%; background:#f5f5f5;">
    ...
    <label for="<?php echo $this->get_field_id( 'post-excerpt-length' ); ?>"><?php _e('Excerpt length', 'recently'); ?>: <input type="text" id="<?php echo $this->get_field_id( 'post-excerpt-length' ); ?>" name="<?php echo $this->get_field_name( 'post-excerpt-length' ); ?>" value="<?php echo esc_attr($instance['post-excerpt']['length']); ?>" class="widefat" style="width:50px!important" /></label><br  />
...
<div class="date-formats" style="display:<?php if ( $instance['meta_tag']['date']['active'] ) : ?>block<?php else: ?>none<?php endif; ?>; width:90%; margin:10px 0; padding:3% 5%; background:#f5f5f5;">
    ...
    <label title='<?php echo get_option('date_format'); ?>'><input type='radio' name='<?php echo $this->get_field_name( 'date_format' ); ?>' value='wp_date_format' <?php echo ( $instance['meta_tag']['date']['format'] == 'wp_date_format' ) ? 'checked="checked"' : ''; ?> /><?php echo date_i18n(get_option('date_format'), time()); ?></label> <small>(<a href="<?php echo esc_url(admin_url('options-general.php')); ?>" title="<?php _e('WordPress Date Format', 'recently'); ?>" target="_blank"><?php _e('WordPress Date Format', 'recently'); ?></a>)</small><br />
...
<?php
    if ( $taxonomies ) {
        foreach ( $taxonomies  as $taxonomy ) {
            ...
            echo '<label><input type="checkbox" class="checkbox" name="' . $this->get_field_name( 'taxonomy_name' ) . '[]" value="' . esc_attr($taxonomy->name) . '"' . ( in_array($taxonomy->name, $instance['meta_tag']['taxonomy']['names']) ? ' checked="checked"' : '' ) . '> ' . esc_html($taxonomy->labels->name) . '</label><br>';
        }
    }
?>
</div>
...
<div class="custom-html" style="display:<?php if ($instance['markup']['custom_html']) : ?>block<?php else: ?>none<?php endif; ?>; width:90%; margin:10px 0; padding:3% 5%; background:#f5f5f5;">
    ...
    <p>
        ...
        <input type="text" id="<?php echo $this->get_field_id( 'title-start' ); ?>" name="<?php echo $this->get_field_name( 'title-start' ); ?>" value="<?php echo esc_attr($recently_title_start); ?>" class="widefat" style="width:49%!important" />
        <input type="text" id="<?php echo $this->get_field_id( 'title-end' ); ?>" name="<?php echo $this->get_field_name( 'title-end' ); ?>" value="<?php echo esc_attr($recently_title_end); ?>" class="widefat" style="width:49%!important" />
    </p>
